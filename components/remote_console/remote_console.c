...
static const char *RC_BANNER =
    "\r\n"
    "==============================================\r\n"
    "  ESP32 Ethernet-WiFi Bridge - Remote Console\r\n"
...
esp_err_t remote_console_set_bind(uint8_t bind) {
    ...
    rc_config.bind = bind & (RC_BIND_AP | RC_BIND_ETH);
    save_config();

    char bind_str[32] = "";
    if (rc_config.bind & RC_BIND_AP) strcat(bind_str, "AP ");
    if (rc_config.bind & RC_BIND_ETH) strcat(bind_str, "ETH ");
...
esp_err_t remote_console_get_status(remote_console_status_t *status) {
    ...
    if (rc_state.state == RC_STATE_ACTIVE) {
        memcpy(status->client_ip, rc_state.client_ip, sizeof(status->client_ip));
...
static esp_err_t load_config(void) {
    ...
    if (nvs_get_u8(nvs, NVS_KEY_BIND, &u8_val) == ESP_OK) {
        ...
        if (u8_val <= 2) {
            const uint8_t migrate[] = {RC_BIND_AP | RC_BIND_ETH, RC_BIND_AP, RC_BIND_ETH};
            rc_config.bind = migrate[u8_val];
        } else {
            rc_config.bind = u8_val & (RC_BIND_AP | RC_BIND_ETH);
        }
